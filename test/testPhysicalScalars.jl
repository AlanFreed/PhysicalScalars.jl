...
const
    # set! is exported by both MutableTypes and PhysicalScalars, so one needs to specify which is to be used.
    set! = PhysicalScalars.set!
    # toString is exported by all 3 imports, so one needs to specify which is to be used.
...
function run()
    ...
    set!(σ, 1.234)
    y  = MReal(0.001234)
    dϵ = PhysicalScalar(y, SI_STRAINRATE)
    ...
    println(" a - b = ", toString(a-b; format, precision, aligned))
    println(" a * b = ", toString(a*b; format, precision, aligned))
    println("√(b*b) = ", toString(sqrt(b*b); format, precision, aligned))
    println(" a / b = ", toString(a/b; format, precision, aligned))
    println()
